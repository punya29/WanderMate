...
const ManageTravelPackages = () => {
  ...
  const uploadImagesToCloudinary = async (newImages) => {
    const cloudinaryUrl =
      "https://api.cloudinary.com/v1_1/mufin/image/upload";
    const uploadPreset = "d3sabu3n";
  ...
  const handleSubmit = async (e) => {
    ...
    const uploadData = async () => {
      try {
        ...
        console.error("Error adding travelPackages:", err);
        setLoading(false);
      }
    };
    ...
    const editData = async () => {
      try {
        const response = await axios.put(
          `http://localhost:5126/api/Hotel/${id}`,
          hotelData
        );
    ...
    >
      <div className="bg-white p-6 rounded shadow-md">
        <h1 className="text-3xl font-bold mb-4">
          {isEditing ? "Edit Hotel" : "Add New Travel Package"}
        </h1>
        <form onSubmit={(e) => handleSubmit(e)}>
          ...
          >
            {isEditing ? "Update Hotel" : "Add Travel Package"}
          </button>
        </form>
            ...
            <tbody>
              {hotels?.map((hotel, index) => (
                <tr key={index}>
                  ...
                  <td className={`border px-4 py-2 ${hotel.freeCancellation? 'text-green-600':'text-red-600'} font-medium`}>
                    {hotel.freeCancellation ? "Yes" : "No"}
                  </td>
                  <td className={`border px-4 py-2 text-green-600 font-medium ${hotel.reserveNow? 'text-green-600   ':'text-red-600'}`}>
